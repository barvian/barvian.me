	...
	>
		...
		{#if d}
			{#each Array(Math.floor(d / 2 / 2)) as _, i}
				<Slice class={i === 0 ? 'tipjar-slice-shadow' : ''} {r} {w} {h} {d} z={d / 2 - i * 2} />
			{/each}
		{/if}
...
<style>
	...
	[data-wrapper] :global(*),
	[data-wrapper] :global(*::before),
	[data-wrapper] :global(*::after) {
	...
	[data-opening] {
		...
		background: linear-gradient(
			to bottom,
			hsl(var(--color-hue), var(--color-saturation), calc(var(--color-lightness) - 12%)),
			hsl(var(--color-hue), var(--color-saturation), calc(var(--color-lightness) - 20%))
		);
		border-radius: 100%;
		opacity: 0.5;
     ...
     */
		transform: translate3d(-50%, calc(-50% - 1px), calc(var(--depth) * 1px / -2)) rotateX(90deg)
			scaleX(clamp(0.1, var(--hole-space) / var(--hole), 1));
